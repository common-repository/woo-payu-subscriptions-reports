...
class Woo_Payu_Subscriptions_Reports_Admin
{
    ...
    public function woo_payu_subscriptions_reports_menu()
    {
        ...
        add_menu_page(woo_payu_subscriptions_reports()->name, woo_payu_subscriptions_reports()->name, 'manage_options', 'menus' . woo_payu_subscriptions_reports()->nameFormatted(), array($this,'menu' . woo_payu_subscriptions_reports()->nameFormatted()), 'dashicons-media-spreadsheet', 6);
    ...
    public function generateReport($idsSubscriptions)
    {
        ...
        $tituloReporte = "Hoja suscripciones";
        ...
        foreach ($idsSubscriptions as $idsSubscription){

            $idSubscription = $idsSubscription->ID;

            $subscription = new WC_Subscription($idSubscription);
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('D'.$i, $subscription->get_billing_first_name() ? $subscription->get_billing_first_name() . " " . $subscription->get_billing_last_name() : $subscription->get_shipping_first_name() . " " . $subscription->get_shipping_last_name())
                ->setCellValue('E'.$i, $subscription->get_billing_address_1() ? $subscription->get_billing_address_1() . " " . $subscription->get_billing_address_2() : $subscription->get_shipping_address_1() . " " . $subscription->get_shipping_address_2())
                ->setCellValue('F'.$i, $subscription->get_customer_order_notes() ? $subscription->get_customer_order_notes() : $subscription->get_customer_note());
